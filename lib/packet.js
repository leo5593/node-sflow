...
internal.defaultFlowRecords = function (flow, buf) {
    switch (flow.format) {
        case 1:
            ...
            flow.srcMac = buf.toString('hex',24,30);
            flow.dstMac = buf.toString('hex',30,36);
            flow.frameType = buf.toString('hex',36,38);
            if(flow.frameType == "8100")
                flow.vlan = buf.readUInt16BE(38) & 0x0FFF;
            break;
        case 2:
            flow.type = "ethernet";
    ...
    for (buf = buf.slice(4);n;n--) {
        ...
        switch (cnt.format) {
            case 1:
                ...
                cnt.ifSpeed = buf.readUInt32BE(16)*0x100000000 + buf.readUInt32BE(20);
                cnt.ifDirection = buf.readUInt32BE(24); 
                cnt.ifDirectionText = [null,"full-duplex","half-duplex","in","out"][cnt.ifDirection]||"unknown";
                cnt.ifStatus = buf.readUInt32BE(28);
                cnt.ifStatusAdmin = (cnt.ifStatus&1)?"up":"down";
                cnt.ifStatusOper = (cnt.ifStatus&2)?"up":"down";
                cnt.ifInOctets = buf.readUInt32BE(32)*0x100000000 + buf.readUInt32BE(36);
                cnt.ifInUcastPkts = buf.readUInt32BE(40);
                cnt.ifInMulticastPkts = buf.readUInt32BE(44);
                cnt.ifInBroadcastPkts = buf.readUInt32BE(48);
                cnt.ifInDiscards = buf.readUInt32BE(52);
                cnt.ifInErrors = buf.readUInt32BE(56);
                cnt.ifInUnknownProtos = buf.readUInt32BE(60);
                cnt.ifOutOctets = buf.readUInt32BE(64)*0x100000000 + buf.readUInt32BE(68);
                cnt.ifOutUcastPkts = buf.readUInt32BE(72);
                cnt.ifOutMulticastPkts = buf.readUInt32BE(76);
                cnt.ifOutBroadcastPkts = buf.readUInt32BE(80);
                cnt.ifOutDiscards = buf.readUInt32BE(84);
                cnt.ifOutErrors = buf.readUInt32BE(88);
                cnt.ifPromiscousMode = buf.readUInt32BE(92);
                break;
            case 2:
